...
To illustrate that $R^*$ provides a reliable metric for capturing convergence, we now calculate a quantitative measure that captures how close the sampling distribution is to the target. One measure of distributional "closeness" is the KL-divergence, that in this case could measure the divergence from the target to the sampling distribution: if the target distribution is known, fitting a kernel density estimator (KDE) to samples allows an approximate (typically univariate) measure of KL-divergence to be calculated. The trouble is, for distributions like the Cauchy that have fat tails, fitting a KDE to the samples provides a noisy measure of the sampling distribution in the tails. This means that approximate KL-divergence is unreliable for these types of model. Instead, we choose a measure of distributional discrepancy based around similarity between target quantiles and sample-derived quantities. Specifically, we calculate the $R^2$ for the regression of the actual quantile values on the sample-estimated quantiles, where, if $R^2\sim 1$, the sampling distribution recapitulates well the target quantities. In our example, we consider all percentiles: 0.1\%, 0.2\%,...,99.8\%, 99.9\% and calculate the mean $R^2$ across all 50 $x_j$.

In the left-hand panel of Fig. \ref{fig:cauchy_convergence}, we plot this ``quantile $R^2$'' as a function of MCMC sample size for both the nominal and alternative parameterisations of the Cauchy model. This shows that after only c.10,000 iterations the alternative parameterisation approaches $R^2\approx 1$; at the same number of iterations, the nominal parameterisation still provides a poor measure of tail quantiles. In the middle and right panels of Fig. \ref{fig:cauchy_convergence}, we show how the maximum value $\hat{R}$ across all 50 dimensions of the target and $R^*$ as calculated by Algorithm \ref{alg:R_star} (both measures are calculated on chains split into equal halves) perform as the number of MCMC samples increases. After c.550 iterations, the alternative parameterisation achieves split-$\hat{R}<1.01$ for all target dimensions, and, after c.10,400 iterations, the nominal model achieves the same maximum split-$\hat{R}$ value: in both cases, these suggest convergence. By contrast, after 25,000 iterations, $R^*\approx 1.05$ for the nominal model, and $R^*>2$ for the nominal parameterisation: in both cases, these $R^*$ values suggest lack of convergence. 

\begin{figure}[!htb]
	\centerline{\includegraphics[width=1.0\textwidth]{../output/cauchy_convergence.pdf}}
	\caption{\textbf{Measuring convergence with MCMC sample size for Cauchy model.} The left-hand panel shows a measure of convergence, the mean quantile $R^2$, that requires knowing the target distribution; the middle panel, shows the maximum value of split-$\hat{R}$ across each of the 50 dimensions of the target; the right shows the value of split-$R^*$ as calculated by Algorithm \ref{alg:R_star}. See the R markdown file on the Github repository for code to reproduce this example.}
