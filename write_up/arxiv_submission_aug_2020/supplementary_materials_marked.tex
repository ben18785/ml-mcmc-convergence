...
In this section, we investigate how classification accuracy depends on choice of ML classifier. 
...
In this section, we revisit the alternative parameterisation of the Cauchy model described in \S\ref{sec:cauchy} and investigate how $R^*$ as calculated by GBM and RF models depends on hyperparameters. Note, that for the GBM model, we did not investigate a full range of hyperparameters needed to optimise $R^*$ due to the extensive training time needed to do so. For each combination of model and hyperparameters, we generated samples from the model as described in \S\ref{sec:cauchy} and performed 20 replicates at each of these combinations.
...
In this section, we revisit the non-centered parameterisation of the eight schools model described in \S\ref{sec:eight_shools}. We investigated a range of hyperparameters for GBM and RF classifiers. At each combination of classifier and hyperparameter setting, we performed 50 replicates: in each replicate, we used Stan's NUTS algorithm with adapt delta set to 0.95 to generate posterior samples; these were then used to calculate $R^*$.
