...
laccuracy <- future_map_dbl(iter, function(i) r_star(x[1:i, , ], method="xgbTree"))

fit <- readRDS("../output/fit_alt_50000.rds")
x <- rstan::extract(fit, permuted=F, pars="x")

laccuracy1 <- future_map_dbl(iter, function(i) r_star(x[1:i, , ], method="xgbTree"))

a_df <- 
  tibble(iteration=iter,
         alt=laccuracy1,
       nom=laccuracy)
saveRDS(a_df, "../output/cauchy_r_star_xgbtree.rds")
```
...
Add in GBM R*
```{r}
a_df <- readRDS("../output/cauchy_r_star_xgbtree.rds") %>% 
  mutate(model="gbm")
...
ggsave("../output/fig_9d.pdf", g2, width = 12, height = 8)
```
