...
g1 <- b_df %>% 
  ...
  ylim(NA, 1) +
  ylab(TeX("$R^2$")) +
  scale_color_brewer("Parameterisation", palette = "Dark2", labels=c("Alternative", "Nominal")) +
  xlab("Iteration") +
...
g3 <- 
  ...
  ylab(TeX("$\\hat{R}$")) +
  scale_color_brewer("Parameterisation", palette = "Dark2", labels=c("Alternative", "Nominal")) +
  scale_fill_brewer("Parameterisation", palette = "Dark2", labels=c("Alternative", "Nominal")) +
  xlab("Iteration") +
  theme(text=element_text(size=11, colour="black"),
        axis.text = element_text(size=10, colour="black"),
        legend.position = "none",
  ...
  ggtitle(TeX("Max split-$\\hat{R}$"))
g3
```

Combine
```{r}
pdf("../output/cauchy_convergence.pdf", width = 12, height = 4)
multiplot(g1, g3, g2, cols = 3)
dev.off()
```
