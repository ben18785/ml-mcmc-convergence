...
temp_df <- 
  ...
  mutate(variable=if_else(variable=="rstar_rf", "rf", if_else(variable=="rstar_xgbTree", "xgbtree", "optimal")))
saveRDS(temp_df, "../data/xgbTree_vs_rf_normals_results_optimals.rds")
temp_df <- readRDS("../data/xgbTree_vs_rf_normals_results_optimals.rds")
g <- ggplot(data=filter(temp_df, variable!="optimal"),
       aes(x=as.factor(d), y=middle, colour=as.factor(variable))) +
  geom_pointrange(aes(ymin=lower, ymax=upper),
                  position = position_dodge2(width=0.3)) +
  geom_hline(yintercept = 1, linetype=2) +
  geom_point(data=filter(temp_df, variable=="optimal")) +
  scale_color_brewer("Method", palette = "Dark2") +
  xlab("# dimensions") +
  ylab("R*") +
  theme(text = element_text(size=16, colour="black"),
        axis.text = element_text(colour="black"))
ggsave("../output/xgbtree_vs_rf_normal.pdf", g, width = 10, height = 6)
```
...
r_star_opts1 <- map_dbl(seq(1, nrow(m_parameters), 1), ~f_optimal_studentts(1000, nu1, m_parameters$nu2[.], m_parameters$ds[.], A0))
...
temp_df <- 
  ...
  mutate(variable=as.character(variable)) %>% 
  mutate(variable=if_else(variable=="rstar_rf", "rf", if_else(variable=="rstar_xgbTree", "xgbtree", "optimal")))
saveRDS(temp_df, "../data/xgbTree_vs_rf_studentts_results_optimal.rds")
temp_df <- readRDS("../data/xgbTree_vs_rf_studentts_results_optimal.rds")
g2 <- ggplot(data=filter(temp_df, variable!="optimal"),
  ...
  theme(text = element_text(size=16, colour="black"),
        axis.text = element_text(colour="black")) +
  facet_wrap(~d)
ggsave("../output/xgbtree_vs_rf_studentt.pdf", g2, width = 12, height = 8)
```
