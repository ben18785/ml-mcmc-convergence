...
g <- r_star_vals %>% 
  ...
  annotate("text", x = b_sum$x_pos, y = b_sum$y_pos, label = b_sum$below_per, size=5) +
...
for(j in seq_along(lnum_chains)){
  ...
  saveRDS(mResults, "../data/ar1_numchains_extra1.rds")
}
...
g <- mResults %>% 
  ...
  theme(axis.text = element_text(colour="black", size=12),
        ...
        legend.position = "none")
ggsave("../output/ar1_numchains.pdf", g, width = 12, height = 8)
```
