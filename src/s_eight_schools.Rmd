...
both <- f_fit_gbm_predict(m_flattened)
...
# Plot tau prediction accuracy across quantiles
```{r}
full_data_all <- rstan::extract(fit_cp, permuted=F)
full_data <- array(dim=c(500, 8, 11))
k <- 1
...
f_r_star_accuracy <- function(nsim, pred_test_prob, testing_data){
...
nsim <- 20
mAccuracy <- f_r_star_accuracy(nsim, plda, testing_data)
colnames(testing_data) <- c("mu", "tau",
                            "theta.1", "theta.2",
                            "theta.3", "theta.4",
                            "theta.5", "theta.6",
                            "theta.7", "theta.8",
                            "lp", "chain")
colnames(mAccuracy) <- map_chr(seq(1, nsim, 1), ~paste0("V", .))
mAccuracy <- mAccuracy %>% 
  as.data.frame() %>% 
  bind_cols(testing_data)
  
lbreaks <- quantile(mAccuracy$tau, seq(0, 1, 0.1))
mAccuracy$tau_cuts <- cut(mAccuracy$tau, breaks=lbreaks, include.lowest = T, right=T, ordered_result = T)
cuts <- sort(unique(mAccuracy$tau_cuts))
r_star <- matrix(ncol = length(cuts),
                 nrow=nsim)
for(i in seq_along(cuts)){
  temp <- mAccuracy %>% 
    filter(tau_cuts==cuts[i])
...
ggplot(filter(r_full, !is.na(Var1)),
  ...
  geom_jitter(height = 0, width = 0.02, colour="grey") +
  ylab(TeX("$R*$")) +
  xlab("Quantile") +
