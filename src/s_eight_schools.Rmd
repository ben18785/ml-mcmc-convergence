...
mon_ncp <- monitor_extra(split_data(x))

c_df <- tibble(centered=r_star_centered,
               non_centered=r_star_noncentered,
               metric="R*",
               simulations="split")
c1_df <- tibble(centered=mon_cp$zfsRhat[1:10],
                non_centered=mon_ncp$zfsRhat[c(1, 2, 11:18)],
                metric="split-Rhat",
               simulations="split")
```
...
```{r}
x <- rstan::extract(fit_cp, permuted=F)
r_star_centered <- r_star_dist(x, split_chains = F)
mon_cp <- monitor_extra(fit_cp)
# noncentered
x <- rstan::extract(fit_ncp, permuted=F)
r_star_noncentered <- r_star_dist(x, split_chains = F)
mon_ncp <- monitor_extra(fit_ncp)

d_df <- tibble(centered=r_star_centered,
               non_centered=r_star_noncentered,
               metric="R*",
               simulations="unsplit")
d1_df <- tibble(centered=mon_cp$zfsRhat[1:10],
                non_centered=mon_ncp$zfsRhat[c(1, 2, 11:18)],
                metric="split-Rhat",
               simulations="unsplit")
```
...
g3 <- e_df %>%
  ...
  geom_point() +
  scale_x_continuous(limits=c(NA, 1.035)) +
  scale_y_continuous(limits=c(NA, 1.035)) +
...
plot_grid(arrangeGrob(gb, top="A. mu"), arrangeGrob(ga, top="B. tau"), g1, g2,nrow = 2, rel_heights = c(1/2, 1/4))
dev.off()
```
