...
ess_b <- a_sum$Bulk_ESS
ess_t <- a_sum$Tail_ESS
...
b_df <- tibble(samples_10000_cp=a_sum$Rhat[1:250])
```
...
a_sum <- monitor(fit)
b_df <- b_df %>%
  mutate(samples_400=a_sum$Rhat[1:250])
```
...
a_sum <- monitor(fit)
sum(a_sum$Bulk_ESS>400)
sum(a_sum$Tail_ESS>400)
b_df <- b_df %>%
  mutate(samples_10000_ncp=a_sum$Rhat[251:500])
