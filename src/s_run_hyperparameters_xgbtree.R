...
f_replicate_hyper <- function(m_parameters, f_data_generator){
  ...
  for(i in 1:nrow(m_parameters))
    laccuracy[i] <- r_star(a_array,
                           caret_default=tibble(nrounds = m_parameters$nrounds[i],
                                                max_depth = m_parameters$max_depth[i],
                                                eta = m_parameters$eta[i],
                                                gamma = m_parameters$gamma[i],
                                                colsample_bytree = m_parameters$colsample_bytree[i],
                                                min_child_weight = m_parameters$min_child_weight[i],
                                                subsample = m_parameters$subsample[i]),
                           method="xgbTree")
  return(laccuracy)
}
...
if(model_num==1){
  m_parameters <- expand_grid(nrounds = c(1, 10, 100),
                              max_depth = c(1, 4, 8),
                              eta = c(.1, .4),
                              gamma = 0,
                              colsample_bytree = .7,
  ...
  m_parameters <- m_parameters <- expand_grid(nrounds = c(10, 100, 400),
                                              max_depth = c(1, 4, 8),
                                              eta = c(.1, .4),
                                              gamma = 0,
                                              colsample_bytree = .7,
...
saveRDS(list(r_star=r_star_vals, hyperparameters=m_parameters), paste0("../data/hypers_xgbtree_", name, ".rds"))