...
f_replicate_hyper <- function(m_parameters, f_data_generator){
  ...
  for(i in 1:nrow(m_parameters))
    laccuracy[i] <- r_star(a_array,
                           caret_default=expand.grid(mtry=m_parameters$mtry[i]))
  return(laccuracy)
}
...
if(model_num==1){
  m_parameters <- expand_grid(mtry=c(1, 2))
  ...
  m_parameters <- expand_grid(mtry=seq(8, 16, 2))
...
saveRDS(list(r_star=r_star_vals, hyperparameters=m_parameters), paste0("../data/hypers_rf_", name, ".rds"))