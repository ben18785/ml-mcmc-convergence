...
a_df <- tibble(samples_5000=colMeans(mAccuracy)/(1/n_distinct(training_data$chain)))
```
...
g <- 
  a_df %>% 
  select(samples_400, samples_10000_ncp) %>% 
  melt() %>% 
  ggplot(aes(x=value, fill=as.factor(variable))) +
  geom_histogram(position="identity", alpha=0.8, bins=20) +
  scale_fill_grey("# of samples", labels=c("400", "2000", "5000")) +
  xlab(TeX("$R*$")) +
  ylab("Count") +
  geom_vline(xintercept = 1, linetype=2) +
  theme(text = element_text(size=14, colour="black"))
# ggsave("../output/mvt_three.pdf", g, width = 8, height = 6)
```
