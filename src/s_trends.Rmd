...
g <- 
  res %>%
  select(iters, trend, Rstar, Rstar_split) %>% 
  melt(id.vars=c("iters", "trend")) %>% 
...
g <- readRDS("../output/experiment_trends_one_dim.rds") %>% 
  select(trend, iters, rep, dimension, Rstar_split) %>% 
  unique() %>%
  select(-rep) %>% 
  melt(c("trend", "iters", "dimension")) %>% 
...
for (i in 1:nrow(conds)) {
 ...
 r_2d <- matrix(nrow = iters * chains, ncol=(2 + 2))
  ...
  res[[i]] <- cbind(iters, rep, rho, t(rs), "Rstar"=a_accuracy * n_distinct(r1$chain),
                    "Rstar_split"=b_accuracy * n_distinct(r2$chain))
}
...
g <- readRDS("../output/experiment_trends_joint_distribution.rds") %>%
  as.data.frame() %>% 
  select(iters, rho, rep, zsRhat, Rstar_split) %>% 
  unique() %>%
  select(-rep) %>% 
  melt(c("rho", "iters"))  %>% 
...
ggplot(data = res, aes(x = rho, y = zsRhat)) + 
  geom_jitter() + 
  facet_grid(vars(iters)) + 
  geom_hline(yintercept = 1) + 
  ggtitle('R* without splitting') +
  ylim(0, 6)

ggplot(data = res, aes(x = rho, y = Rstar_split)) + 
  geom_jitter() + 
  facet_grid(vars(iters)) + 
  geom_hline(yintercept = 1) + 
  ggtitle('Split-R*') +
