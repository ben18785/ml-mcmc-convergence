...
```{r}
res <- readRDS("../output/experiment_trends_one_dim.rds") %>% 
  select(trend, iters, rep, dimension, Rstar) %>% 
  unique()
ggplot(data = res, aes(x = trend, y = Rstar)) + 
  geom_point() + 
  geom_jitter() + 
...
res <- readRDS("../output/experiment_trends_one_dim.rds") %>% 
  ...
  unique()
ggplot(data = res, aes(x = trend, y = Rstar_split)) + 
  geom_point() + 
  geom_jitter() + 
  facet_grid(vars(iters), vars(dimension)) + 
  geom_hline(yintercept = 1) + 
...
ggplot(data = res, aes(x = rho, y = Rstar)) + 
  geom_jitter() + 
