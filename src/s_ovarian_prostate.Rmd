...
g <- tibble(long=r_star_longer, short=r_star_shorter, metric="A. R*") %>% 
  bind_rows(tibble(long=mon_longer$Rhat, short=mon_shorter$Rhat, metric="B. Rhat")) %>% 
  bind_rows(tibble(long=mon_longer$Bulk_ESS, short=mon_shorter$Bulk_ESS, metric="C. Bulk ESS")) %>% 
  bind_rows(tibble(long=mon_longer$Tail_ESS, short=mon_shorter$Tail_ESS, metric="D. Tail ESS")) %>% 
  melt(id.vars="metric") %>% 
  mutate(metric=fct_relevel(as.factor(metric), "A. R*", "B. Rhat", "C. Bulk ESS", "D. Tail ESS")) %>% 
  ggplot(aes(x=value, fill=as.factor(variable))) +
  geom_histogram(position="identity", alpha=0.6) +
  facet_wrap(~metric, scales="free") +
  scale_fill_grey("MCMC info.", labels=c("4 chains: long", "16 chains: short")) +
...
# Thin data again
r_star <- r_star_dist(full_data[, , seq(1, 18106, 5)])
