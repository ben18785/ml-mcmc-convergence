...
g <- readRDS("../data/hypers_ar1.rds")$r_star %>% 
  ...
  scale_y_continuous(limits=c(0.98, NA)) +
  geom_hline(yintercept = 1, linetype=2) +
  scale_color_brewer("Int. depth", palette = "Dark2") +
  guides(colour = guide_legend(nrow = 2, byrow = T))
ggsave("../output/hypers_ar1.pdf", g, width = 12, height = 8)
```
...
readRDS("../data/hypers_8_schools.rds")$r_star %>% 
  melt(id.vars=c("int.depth", "n.trees")) %>% 
  group_by(int.depth, n.trees) %>% 
  summarise(lower=quantile(value, 0.25),
            middle=quantile(value, 0.5),
...
readRDS("../data/hypers_cauchy.rds")$r_star %>% 
  melt(id.vars=c("int.depth", "n.trees")) %>% 
  group_by(int.depth, n.trees) %>% 
  summarise(lower=quantile(value, 0.25),
            middle=quantile(value, 0.5),
...
AR1
```{r}
temp <- readRDS("../data/hypers_rf_ar1.rds")$r_star
temp %>%
  melt(id.vars="mtry") %>%
  group_by(mtry) %>%
  summarise(lower=quantile(value, 0.25),
...
8 schools
```{r}
temp <- readRDS("../data/hypers_rf_8_schools.rds")$r_star
temp %>%
  melt(id.vars="mtry") %>%
  group_by(mtry) %>%
  summarise(lower=quantile(value, 0.25),
...
Cauchy
```{r}
temp <- readRDS("../data/hypers_rf_cauchy.rds")$r_star
temp %>%
  melt(id.vars="mtry") %>%
  group_by(mtry) %>%
  summarise(lower=quantile(value, 0.25),
...
Normal
```{r}
temp <- readRDS("../data/hypers_rf_")$r_star
temp %>%
  melt(id.vars="mtry") %>%
  group_by(mtry) %>%
  summarise(lower=quantile(value, 0.25),
...
AR1
```{r}
temp <- readRDS("../data/hypers_xgbtree_ar1.rds")$r_star
temp %>%
  select(-one_of("gamma", "colsample_bytree", "min_child_weight", "subsample")) %>% 
  melt(id.vars=c("nrounds", "max_depth", "eta")) %>%
  group_by(nrounds, max_depth, eta) %>%
  summarise(lower=quantile(value, 0.25),
            middle=quantile(value, 0.5),
            upper=quantile(value, 0.75)) %>%
  ggplot(aes(x=as.factor(nrounds), y=middle, colour=as.factor(max_depth), group=as.factor(max_depth))) +
  geom_pointrange(aes(ymin=lower, ymax=upper), position=position_dodge2(width=0.1)) +
  geom_line() +
  xlab("# rounds") +
  ylab("R*") +
...
```{r}
temp <- readRDS("../data/hypers_xgbtree_8_schools.rds")$r_star
temp %>%
  select(-one_of("gamma", "colsample_bytree", "min_child_weight", "subsample")) %>% 
  melt(id.vars=c("nrounds", "max_depth", "eta")) %>%
  group_by(nrounds, max_depth, eta) %>%
...
```{r}
temp <- readRDS("../data/hypers_xgbtree_cauchy.rds")$r_star
temp %>%
  select(-one_of("gamma", "colsample_bytree", "min_child_weight", "subsample")) %>% 
  melt(id.vars=c("nrounds", "max_depth", "eta")) %>%
  group_by(nrounds, max_depth, eta) %>%
