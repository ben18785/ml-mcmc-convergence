...
colnames(m_res) <- c("gbm", "rhat", "rf", "knn", "svm", "nnet", "multinom")
m_res1 <- m_res %>%
  as.data.frame() %>% 
...
tibble(var1=testing_data$V7, var2=testing_data$V15, chain=plda) %>% 
  ggplot(aes(x=var1, y=var2, colour=as.factor(chain))) +
  geom_jitter() +
  geom_smooth(method="lm", se=F)

tibble(var1=testing_data$V7, var2=testing_data$V15, chain=plda) %>% 
  ggplot(aes(x=var1, colour=as.factor(chain))) +
  stat_density(fill="none", position="identity")

hist(map_dbl(seq(1, 8, 1), function(i) cor(testing_data %>% filter(chain==i) %>% select(-chain) %>% as.matrix())[7, 15]))
```

Visualise between-chain correlation
```{r}
x <- split_data(x_ncp)
cors <- map(seq(1, dim(x)[2], 1), ~cor(x[, ., ]))
m_sd <- matrix(nrow = 19, ncol = 19)
for(i in 1:19)
  for(j in 1:19)
    m_sd[i, j] <- sd(map_dbl(seq(1, dim(x)[2], 1), ~cors[[.]][i, j]))
tmp <- which(m_sd == max(m_sd), arr.ind = TRUE)
a <- tmp[1,1]
b <- tmp[1,2]
m_sd[a, b]
colnames(r)[a] <- "a"
...
g <- r %>% 
  ggplot(aes(x=a, y=b, colour=as.factor(chain))) +
  geom_jitter(alpha=0.1) +
  geom_smooth(method="lm", se=F) +
  scale_color_brewer("Chain", palette = "Dark2")
ggsave("../output/ml_comparison_8_schools_ncp_1.pdf", g, width = 12, height = 8)

g <- r %>% 
  select(chain, a, b) %>% 
  melt(id.vars="chain") %>% 
  ggplot(aes(x=value, colour=as.factor(chain), group=as.factor(chain))) +
  stat_density(fill="none", position = "identity") +
  facet_wrap(~variable)
ggsave("../output/ml_comparison_8_schools_ncp_2.pdf", g, width = 12, height = 8)
...
g <- r %>% 
  ...
  ggplot(aes(x=as.factor(chain), y=value)) +
  geom_point() +
  facet_wrap(~variable)
ggsave("../output/ml_comparison_8_schools_ncp_3.pdf", g, width = 12, height = 8)

g <- r %>% 
  select(chain, a, b) %>% 
  melt(id.vars="chain") %>% 
  group_by(variable, chain) %>% 
  summarise(value=quantile(value, 0.95)) %>% 
  ggplot(aes(x=as.factor(chain), y=value)) +
  geom_point() +
  facet_wrap(~variable)
ggsave("../output/ml_comparison_8_schools_ncp_4.pdf", g, width = 12, height = 8)
```



How R* varies with hyperparameters
```{r}
x <- x_ncp

f_run_gbm <- function(x, n.trees_, int.depth_){
  return(f_replicate_ml_simple(x, "gbm", expand.grid(interaction.depth=c(int.depth_), 
                             n.trees = n.trees_,
                             shrinkage=c(0.1),
                             n.minobsinnode=10)))
}
...
hypers <- tibble(
       n.trees=c(50, 50, 100, 200, 200, 200, 200, 400, 400, 1000),
       int.depth=c(3,5,3,3,5,7,9,5,9,11))
k <- 1
niter <- 20
m_results <- matrix(nrow = nrow(hypers) * niter, ncol=4)
for(i in 1:nrow(hypers)){
  for(j in 1:niter){
    tree <- hypers$n.trees[i]
    depth <- hypers$int.depth[i]
    temp <- f_run_gbm(x, tree, depth)
    m_results[k, ] <- c(tree, depth, j, temp)
    k <- k + 1
  }
}
colnames(m_results) <- c("n.tree", "int.depth", "iter", "r_star")
m_results <- m_results %>% 
  as.data.frame()
saveRDS(m_results, "../data/8_schools_gbm_hyper.rds")
m_results <- readRDS("../data/8_schools_gbm_hyper.rds")
g <- m_results %>% 
  group_by(n.tree, int.depth) %>% 
  summarise(lower=quantile(r_star, 0.25),
            middle=median(r_star),
            upper=quantile(r_star, 0.75)) %>% 
  ggplot(aes(x=as.factor(n.tree), y=middle, color=as.factor(int.depth))) +
  geom_pointrange(aes(ymin=lower, ymax=upper), position = position_dodge2(width=0.2)) +
  ylim(0.95, NA) +
  geom_hline(yintercept = 1, linetype=2) +
  scale_color_brewer("Int.depth", palette = "Dark2") +
  xlab("# tree") +
  ylab("R*")
ggsave("../output/ml_comparison_8_schools_hyper.pdf", g, width = 12, height = 8)
```

Try gbm function itself to optimise hyperparameters
```{r}
x <- x_ncp
x <- split_data(x)
...
gbmFit <- train(chain ~ ., data = training_data, 
       ...
       n.trees=c(50, 200, 2000),
       interaction.depth=c(3,11, 17),
       shrinkage=c(0.1),
             n.minobsinnode=c(10)))
plda <- predict(object=gbmFit, newdata=testing_data)
a_accuracy <- 
...
for(i in 1:niterates){
  ...
  l_svm_hyper[[i]] <- temp$hyper
  m_res[i, 5] <- temp$a_accuracy
  temp <- f_replicate_ml_simple_gridded(x, "multinom", tunegrid_multinom)
  l_nnet_hyper[[i]] <- temp$hyper
  m_res[i, 6] <- temp$a_accuracy
}

colnames(m_res) <- c("gbm", "rhat", "rf", "knn", "svm", "multinom")
m_res1 <- m_res %>%
  as.data.frame() %>% 
  mutate(iter=seq_along(gbm))
saveRDS(m_res1, "../data/ml_comparison_cauchy_nom.rds")
...
g <- m_res1 %>% 
  ...
  ylab("R*") +
  theme(text=element_text(colour="black", size=14))
ggsave("../output/ml_comparison_cauchy_nom.pdf", g, width = 12, height = 8)
...
m_res1 <- m_res %>%
  as.data.frame() %>% 
  mutate(iter=seq_along(gbm))
saveRDS(m_res1, "../data/ml_comparison_cauchy_alt.rds")
m_res1 <- readRDS("../data/ml_comparison_cauchy_alt.rds")
m_res2 <- readRDS("../data/ml_comparison_cauchy_nom.rds") %>% 
  mutate(type="nominal")
...
g <- m_res1 %>% 
  ...
  bind_rows(m_res2) %>% 
  dplyr::select(-one_of("rhat")) %>% 
  melt(id.vars=c("iter", "type")) %>% 
  mutate(value=as.numeric(as.character(value))) %>% 
  ggplot(aes(x=fct_reorder(as.factor(variable), value), y=value, fill=as.factor(type))) +
  geom_boxplot() +
  xlab("ML model") +
  ylab("R*") +
  theme(text=element_text(colour="black", size=14)) +
  scale_fill_brewer("Parameterisation", palette = "Dark2") +
  scale_y_continuous(limits=c(0.8, NA)) +
  geom_hline(yintercept = 1, linetype=2)
ggsave("../output/ml_comparison_cauchy.pdf", g, width = 12, height = 8)
```
